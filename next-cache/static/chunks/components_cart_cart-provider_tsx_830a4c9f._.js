...
function CartProvider(param) {
    ...
    const addItem = (item)=>{
        setItems((prev)=>{
            const existing = prev.find((p)=>p.id === item.id && p.storeSlug === item.storeSlug);
            if (existing) {
                return prev.map((p)=>p.id === item.id && p.storeSlug === item.storeSlug ? {
                        ...p,
    ...
    const removeItem = (id, storeSlug)=>{
        setItems((prev)=>prev.filter((item)=>!(item.id === id && item.storeSlug === storeSlug)));
    };
    const clearCart = (storeSlug)=>{
        if (!storeSlug) {
            setItems([]);
            return;
        }
...
//# sourceMappingURL=components_cart_cart-provider_tsx_830a4c9f._.js.map